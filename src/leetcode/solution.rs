...
mod tests {
  ...
  fn test_exclusive_time() {
    let solution = Solution::new();
    assert_eq!(vec![3, 4], solution.exclusive_time(2, vec!["0:start:0", "1:start:2", "1:end:5", "0:end:6"].iter().map(|&x| x.to_string()).collect()));
    assert_eq!(vec![8], solution.exclusive_time(1, vec!["0:start:0", "0:start:2", "0:end:5", "0:start:6", "0:end:6", "0:end:7"].iter().map(|&x| x.to_string()).collect()));
    assert_eq!(vec![7, 1], solution.exclusive_time(2, vec!["0:start:0", "0:start:2", "0:end:5", "1:start:6", "1:end:6", "0:end:7"].iter().map(|&x| x.to_string()).collect()));
    assert_eq!(vec![8, 1], solution.exclusive_time(2, vec!["0:start:0", "0:start:2", "0:end:5", "1:start:7", "1:end:7", "0:end:8"].iter().map(|&x| x.to_string()).collect()));
    assert_eq!(vec![1], solution.exclusive_time(1, vec!["0:start:0", "0:end:0"].iter().map(|&x| x.to_string()).collect()));
  }
  ...
  fn test_unique_morse_representations() {
    ...
    assert_eq!(solution.unique_morse_representations(vec![String::from("a")]), 1);
  }
  ...
  fn test_shortest_to_char() {
    let solution = Solution::new();
    assert_eq!(solution.shortest_to_char(String::from("loveleetcode"), 'e'), vec![3, 2, 1, 0, 1, 0, 0, 1, 2, 2, 1, 0]);
    assert_eq!(solution.shortest_to_char(String::from("aaab"), 'b'), vec![3, 2, 1, 0]);
  }
